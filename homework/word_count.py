...
import fileinput #permite leer contenidos completod y archivos
import glob
import os.path
...
def load_input(input_directory):
    ...
    with fileinput.input(files=files) as f:  #lee uno por uno y nos evitamos el ciclo for
        for line in f:
...
def shuffle_and_sort(sequence):
    """Shuffle and Sort"""
    return sorted(sequence, key=lambda x: x[0]) # toma sequence y la ordena , el key con lamda x muestra que ordene de primero cual
...
def reducer(sequence):
    """Reducer"""
    #Toma cada clave valor , si no esta en el diccionario lo crea, si ya esta le suma el valor en el diccionario
    #retorna los items
    result = {}
    for key,value in sequence:
